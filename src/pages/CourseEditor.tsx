...
import { fetchCourseData, saveCourseSection } from '../store/slices/courseDataSlice';
...
export default function CourseEditor() {
  ...
  const courseData = useAppSelector((s) => s.courseData);
  ...
  const handleEdit = () => {
    const raw = JSON.stringify(courseData[section as keyof typeof courseData], null, 2);
    setDraft(raw);
    setJsonError(null);
    setSaved(false);
    setEditing(true);
    ...
    <Box>
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent sx={{ display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          >
            {courseData.loading ? <CircularProgress size={16} /> : 'Reload from DB'}
          </Button>
          ...
          {editing && (
            <>
              <Button
                variant="contained"
                color="success"
                startIcon={courseData.saving ? <CircularProgress size={16} sx={{ color: '#fff' }} /> : <SaveIcon />}
                onClick={handleSave}
                disabled={courseData.saving}
              >
                Save to DB
              </Button>
      ...
      {courseData.error && <Alert severity="error" sx={{ mb: 2 }}>{courseData.error}</Alert>}
      ...
      <Card>
        <CardContent sx={{ p: 0, '&:last-child': { pb: 0 } }}>
          ...
          <Box
            component="textarea"
            value={editing ? draft : JSON.stringify(courseData[section as keyof typeof courseData], null, 2)}
