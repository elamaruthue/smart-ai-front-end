...
import { getCourseDataApi, updateCourseSection } from '../../api/courseData';
import type { CourseDataPayload } from '../../api/courseData';
import {
  paths      as localPaths,
...
export const fetchCourseData = createAsyncThunk<
  ...
  void,
  { rejectValue: string }
>('courseData/fetch', async (_, { rejectWithValue }) => {
  try {
    return await getCourseDataApi();
  } catch (err: unknown) {
...
export const saveCourseSection = createAsyncThunk<
  ...
  { section: string; data: unknown },
  { rejectValue: string }
>('courseData/saveSection', async ({ section, data }, { rejectWithValue }) => {
  try {
...
export const { setSectionLocal } = courseDataSlice.actions;
